...
import com.bolsadeideas.ejemplos.util.paginador.Page;
import com.bolsadeideas.ejemplos.util.paginador.render.PagerRender;
...
public class ProductoController {
	...
	public ModelMap listado(@RequestParam(value="page", required=false) String page, ModelMap modelo) throws Exception {
		
		//return productoService.findAll();
		
		Page pager = productoService.findAllPager(page, 4, "");
		
		PagerRender pagerRender = pager.getRender("Bootstrap");
		pagerRender.setRequestUri("listado.htm");
		pagerRender.render();
		
		modelo.addAttribute("productos", pager.getResults());
		modelo.addAttribute("pager", pagerRender.getOutput());
		modelo.addAttribute("titulo", "Listado de Productos con Paginador");
		return modelo;
	}
